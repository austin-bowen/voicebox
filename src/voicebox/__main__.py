...
def _parse_args():
    ...
    parser.add_argument(
        "text", nargs="?", help="Text to speak. If not given, text is read from stdin."
    )
    ...
    parser.add_argument(
        "--ring-mod", action="store_true", help="Add ring modulation effect"
    )
...
def _get_effects(args):
    ...
    if args.vocoder:
        ...
        effects.append(
            Vocoder.build(max_freq=5000, bands=20)
            if args.tts == "picotts"
            else Vocoder.build(max_freq=10000, bands=40)
        )
    ...
    if args.phaser:
        ...
        effects.append(PedalboardEffect(Phaser(rate_hz=0.5, centre_frequency_hz=400)))
